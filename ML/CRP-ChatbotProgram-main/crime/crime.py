...
class Solution(Reader):
    ...
    def save_cctv_pos(self):
        ...
        header = [1]# 자치구 합계 한국인 등록외국인 65세 고령자는 헤더로 취급
        ...
        #pop.dropna(how='all')# 모두 NaN이면 제거
        #pop.dropna(subset=[""])#해당 subset행에 nan값이 있을경우 제거
        #pop.dropna() #NaN값이 있는 모든 행을 삭제
        pop.drop(26, axis=0, inplace=True) # axis=0은 행, axis=1 열
        ...
        print(cctv)
        cctv_pop= pd.merge(cctv,pop, on='구별')#키가 같을땐 merge, 그냥 합칠땐 cctv_pop= pd.concat([cctv,pop])
        print(cctv_pop)
        cor1 = np.corrcoef(cctv_pop['고령자비율'],cctv_pop['소계']) #상관계수
        cor2 = np.corrcoef(cctv_pop['외국인비율'],cctv_pop['소계']) #상관계수(corr)
